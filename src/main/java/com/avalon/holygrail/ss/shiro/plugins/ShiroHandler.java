...
public interface ShiroHandler<T> {
     ...
     * 账户锁定提示
     */
    String LOCKED_ACCOUNT_MESSAGE = "用户名或密码错误";
    /**
     * 无权提示
     */
    String AUTHENTICATION_MESSAGE = "您没有权限做此操作";
    ...
    static boolean subjectLogin(String username, String password) throws ShiroException {
        ...
        try {
            ...
            throw new UnknownAccountException(UNKNOWN_ACCOUNT_MESSAGE, ex);
        } catch (IncorrectCredentialsException ex) {
            throw new IncorrectCredentialsException(INCORRECT_CREDENTIALS_MESSAGE, ex);
        } catch (LockedAccountException ex) {
            throw new LockedAccountException(LOCKED_ACCOUNT_MESSAGE, ex);
        } catch (AuthenticationException ex) {
            throw new AuthenticationException(AUTHENTICATION_MESSAGE, ex);
        }
        return true;
    }
     ...
     * @param response 响应
     */
    void onAccessDenied(ServletRequest request, ServletResponse response);
   ...
   /**
     ...
     * @param response 响应
     */
    void onAjaxAccessDenied(ServletRequest request, ServletResponse response);
}

...
public abstract class AbstractQrCodePay<O> implements QrCodePay<O> {
    ...
    public Map<String, Object> process(String orderId) {
        ...
        switch (orderStatus) {
            ...
            case PRE_CREATED:
                //获取所有注册的第三方支付交易状态
                this.payManager.getTradeStatus(orderId, (payName, tradeStatus) -> {
            ...
            //订单完成
            case SUCCESS:
                return this.doResultsForLocalOrderSuccessStatus(order, results);
            //订单已关闭
            case CLOSED:
                return this.doResultsForLocalOrderClosedStatus(order, results);
            //订单结束
            case FINISHED:
                return this.doResultsForLocalOrderFinishedStatus(order, results);
            default:
